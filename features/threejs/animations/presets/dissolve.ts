...
// vModelPos varying injected into vertex shader (model-space position — always available)
const VERT_VARYING_DECL = `varying vec3 vModelPos;`
const VERT_VARYING_ASSIGN = `vModelPos = position;`
...
function injectDissolve(mat: THREE.Material): Record<string, { value: number }> {
  ...
  m.onBeforeCompile = (shader) => {
    ...
    // Inject model-space position varying into vertex shader
    shader.vertexShader = VERT_VARYING_DECL + '\n' + shader.vertexShader
